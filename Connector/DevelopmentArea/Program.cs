    ...
    class Program
    {
        static async Task Main(string[] args)
        {
            ...
            using (Server server1 = new Server())
            {
                ...
                for (int eventsCounter = 0; eventsCounter < 20; eventsCounter++)
                {
                ...
                foreach (Controllable controllable in server1.Controllables)
                    Console.WriteLine($" * {controllable.Name}");

                Console.WriteLine("EOL");

                await universe1.Part();
                ...
                for (int eventsCounter = 0; eventsCounter < 80; eventsCounter++)
                {
                    Queue<FlattiverseEvent> events = await server0.GatherEvents();
                    ...
                    while (events.TryDequeue(out @event))
                    {
                        ...
                        if (@event is UpdatedUnitEvent)
                            Console.WriteLine($" => DIRECTION={((PlayerUnit)((UpdatedUnitEvent)@event).Unit).Direction}; ROTATION={((PlayerUnit)((UpdatedUnitEvent)@event).Unit).Rotation}; THRUSTER={((PlayerUnit)((UpdatedUnitEvent)@event).Unit).Thruster}");

                        if (@event is HeartbeatEvent)
                        {
                            Console.WriteLine($" * ENERGY={controllable0.Energy}; HULL={controllable0.Hull}/{controllable0.HullMax}; ACTIVE={controllable0.Active}; ALIVE={controllable0.Alive}");

                            if (!controllable0.Alive)
                                await controllable0.Continue();
                        }
                    }
                }
