...
The `propablyKind` information reduces units which are looking quite like the same into the same kinds. We have those groups:
* `sun`.
...
The `team` flag will only tell you, if a unit has the `same` team, a `enemy` team or `none` (no) team. `energyOutput` will specify the energy output of the unit. For suns this will be quite high, for ships without enabled afterburners it will be low, etc...
